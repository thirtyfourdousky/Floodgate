...
public static class TurboAssetManager
{
    ...
    private static string AssetManager_ResolveFilePath_string_bool_bool(On.AssetManager.orig_ResolveFilePath_string_bool_bool orig, string path, bool skipMergedMods, bool skipConsoleFiles)
    {
        if (!accessfgmerged)
        {
        ...
        if (!skipMergedMods)
        {
            string text = Path.Combine(Path.Combine(Custom.RootFolderDirectory(), "floodgatemergedmods"), path.ToLowerInvariant());
            if (File.Exists(text))
            {
                return text;
            }
    ...
    private static List<string> AssetManager_ResolveAllFilePaths(On.AssetManager.orig_ResolveAllFilePaths orig, string path)
    {
        if (!accessfgmerged)
        {
            return orig(path);
    ...
    private static string AssetManager_ResolveDirectory(On.AssetManager.orig_ResolveDirectory orig, string path)
    {
        if (!accessfgmerged)
        {
            return orig(path);
    ...
    private static void AssetManager_ListDirectory_string_bool_bool_bool(ILContext il)
    {
        try
        {
            ILCursor c = new(il);
